...
function myorder() {
  ...
  if (
    mail.length >= 30 || !/^\w+([\.-]?\w+)*@\w+([\.-]?\w+)*(\.\w{2,3})+$/.test(mail)) {
    document.getElementById('spanmail').innerHTML ='**Please enter a valid email address'
  }
   else {
 ...
 //phone validation//
  ...
  if (postcode.length !== 6) {
    document.getElementById('spanpostcode').innerHTML ='** Please enter a 6 digit of post code'
    flag = 1
  }
   else {
